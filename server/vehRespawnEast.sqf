...
_dir = getDir _veh; 
_temp_pos = eastRespawnArray select floor random count eastRespawnArray;
...
if (isDedicated) then {
    While {True} Do {
		...
		if ((_check_pos distance getMarkerPos "east_base_marker") > 30) then {
			...
			_temp_pos = eastRespawnArray select floor random count eastRespawnArray;
			_pos = getMarkerPos _temp_pos;
			diag_log "_________________East Vehicle Respawn markers updated after base reset.";
		} else {
			if ((alive _veh) && {canMove _veh} && {{alive _x} count crew _veh == 0}) then {
				...
				if ((_abandoned) && ((_veh distance _pos) > 100)) then {
					...
					_posMrk = _RandomPosE findEmptyPosition [0, 50, "B_Heli_Light_01_armed_F"];
...
//					"respawn_veh_east" setMarkerPos getPos _veh;
					...
					diag_log "_____________________Vehicle East respawned after being abandoned.";
				};
			};
...
//					"respawn_veh_east" setMarkerPos getPos _veh;
					_veh setVariable ["R3F_LOG_disabled", false, true];
					diag_log "_____________________Vehicle East respawned after being destroyed.";
				};
			};
		};
	};
