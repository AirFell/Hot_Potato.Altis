...
/*
 * Spawns the given object in at the west_veh_shop_spawn1.
...
_spawn_object = {
	...
	_spawn = createVehicle [_name,getPos west_veh_shop_spawn1,[], 0,"CAN_COLLIDE"];
	_spawn setDir (getDir west_veh_shop_spawn1) + 90;
...
{if(_itemText == _x select 0) then{
	sleep 1;
	_ObjectsInArea = [(getPos west_veh_shop_spawn1) select 0, (getPos west_veh_shop_spawn1) select 1] nearObjects 2;
	...
	_ObjectsInArea = _ObjectsInArea + ( [(getPos west_veh_shop_spawn1) select 0, (getPos west_veh_shop_spawn1) select 1] nearObjects ['Car', 10] );
